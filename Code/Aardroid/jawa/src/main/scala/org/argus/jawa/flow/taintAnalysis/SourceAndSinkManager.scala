...
object SourceAndSinkCategory {
  ...
  final val LIBRARY_SOURCE = "library_source" //sources from .aar file, assuming all of these will be
...
trait SourceAndSinkManager[T <: Global] {
  ...
  //this function checks which of the statements/nodes have signature defined in SS file and could be annotated as SS
          ...
          invNode match {
            case _: ICFGCallNode =>
              global.getMethod(invNode.getOwner) match {
                case Some(caller) =>
                  if(callee.callee.methodName.equals("setFullName")) {
                    print("YES")
                  }
                  //println(callee.callee.methodName)
