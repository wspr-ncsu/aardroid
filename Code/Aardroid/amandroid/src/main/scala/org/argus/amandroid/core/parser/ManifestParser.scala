...
class ManifestParser{
  ...
  private var allowBackup = ""
  private var networkSecurityConfig = ""
  ...
  def getAllowBackupFlag() : String ={
    allowBackup
  }

  def getNetworkSecurityConfig() : String ={
    networkSecurityConfig
  }

  def getIntentFilters() : IntentFilter ={
    currentIntentFilter
  }
  ...
  def loadClassesFromTextManifest(manifestIS: InputStream): Unit = {
    try {
      ...
      for (appIdx <- 0 until appsElement.getLength) {
        ...
        // Check whether the application allows backup

        headerNames.foreach {
          header =>
            this.allowBackup = appElement.getAttribute(header + "allowBackup")
            this.networkSecurityConfig = appElement.getAttribute(header + "networkSecurityConfig")
        }
