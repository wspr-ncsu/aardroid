...
case class DecompilerSettings(
  ...
  var libraryInterfaceFile: String = ""
  var interfaceApproach: InterfaceIntegrationApproach.Value = InterfaceIntegrationApproach.MANUAL

  def setLibraryInterfaceFile(libUri: FileResourceUri): Unit = {
    libraryInterfaceFile = libUri
  }

  def setInterfacingApproach(approach: InterfaceIntegrationApproach.Value): Unit = {
    interfaceApproach = approach
  }
}
...
case class DecompileLayout(
  ...
  def ssfile(libraryInterfaceFile : String): String  ={

    var source = scala.io.Source.fromFile("DefaultSourceAndSink.txt")
    if (source == null) source = scala.io.Source.fromFile(AndroidGlobalConfig.settings.sas_file)
    val baseSS = try source.mkString finally source.close()
    val lib = scala.io.Source.fromFile(libraryInterfaceFile)
    val librarySS = try lib.mkString finally lib.close()
    val ss = baseSS+'\n'+librarySS
    val outPath = FileUtil.toFilePath(this.outputSrcUri)
    val path = outPath + File.separator +"SS.txt"
