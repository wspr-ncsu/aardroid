...
//identify if the source is from a .aar file based on the taint tag
class AARDroidSourceAndSinkManager(sasFilePath: String) extends DataLeakageAndroidSourceAndSinkManager(sasFilePath){

  override def isSourceMethod(global: ApkGlobal, sig: Signature): Option[(String, ISet[SSPosition])] = {

    var s :Option[(String, ISet[SSPosition])] = null
    val srcs = this.sources.filter(source => matches(global, sig, source._1))
    val poss = srcs.map(_._2._1).fold(isetEmpty)(iunion)
    if(srcs.nonEmpty && srcs.head._2._2.contains("AAR_SOURCE")) {
      print("Iside this")
      s =Some((SourceAndSinkCategory.LIBRARY_SOURCE, poss))
      print("Created")

    }
    else {
      //print("Inside else")
      s = super.isSourceMethod(global, sig)
    }
    s
  }
}
