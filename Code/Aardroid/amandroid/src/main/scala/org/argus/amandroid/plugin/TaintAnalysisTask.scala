...
import org.argus.amandroid.alir.taintAnalysis.{AARDroidSourceAndSinkManager, DataLeakageAndroidSourceAndSinkManager, DefaultAndroidSourceAndSinkManager}
...
object InterfaceIntegrationApproach extends Enumeration {
  val AUTOMATIC, MANUAL = Value
}

case class TaintAnalysisTask(module: TaintAnalysisModules.Value, fileUris: ISet[(FileResourceUri, FileResourceUri)], forceDelete: Boolean, reporter: Reporter, guessPackage: Boolean, approach: TaintAnalysisApproach.Value, interfacingApproach: InterfaceIntegrationApproach.Value, libraryInterfaceFile: String = "") {

  import TaintAnalysisModules._

  //  private final val TITLE = "TaintAnalysisTask"
  def run: Set[ApkGlobal] = {
    ...
    val apks = fileUris.map { case (apkUri, outputUri) =>
      ...
      if (libraryInterfaceFile.length() > 1) {
        ssfile = layout.ssfile(libraryInterfaceFile)
        settings.setLibraryInterfaceFile(libraryInterfaceFile)
        settings.setInterfacingApproach(interfacingApproach)
      }
      val apk = yard.loadApk(apkUri, settings, collectInfo = true, resolveCallBack = true, guessPackage)
      apk
    ...
    val ssm = module match {
      ...
      case AARDROID =>
        new DefaultAndroidSourceAndSinkManager(ssfile)
    }
    approach match {
