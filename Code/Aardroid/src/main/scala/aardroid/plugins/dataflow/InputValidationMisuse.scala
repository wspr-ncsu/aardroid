...
/**
  ...
  */
class InputValidationMisuse extends MASVSChecker{
  override def name: String = "InputValidationMisuse"
  ...
  private final val validationMethods: ISet[String] = Set(

    "matches"

  )

  def isValidated(tp: TaintPath): Boolean ={

    var sanitized = false
    for(em<-validationMethods){
  ...
  def intermediateMethodExists (tp:TaintPath, signature: String): Boolean={
    ...
    for(n <- tp.getPath){
      try{
        ...
        case e:Exception=>{
          print("Exception in determining input validation method for "+tp)
        }
      }
  ...
  override def check(global: ApkGlobal, idfgOpt: Option[InterProceduralDataFlowGraph]): AnalysisResult = {
    ...
    paths.foreach{p =>
      ...
      dataPersistenceSinks.foreach{s =>
        if (snk.contains(s)){
          val doesValidate = isValidated(p)
          if (!doesValidate){
            ...
            result((p.toString)) = "DF3 violated. Persistence of sensitive data without input validation in local storage found"+sensitivity
          }
        }
