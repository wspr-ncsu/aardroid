...
/**
  ...
  */
class DataPersistenceMisuse extends MASVSChecker{
  override def name: String = "DataPersistenceMisuse"
    ...
    paths.foreach{p =>
      ...
      dataPersistenceSinks.foreach{s =>
        if (snk.contains(s)){
          ...
          result((p.toString)) = "DF1 violated. Persistence of sensitive data in local storage found"+sensitivity
          print(result)
        }
