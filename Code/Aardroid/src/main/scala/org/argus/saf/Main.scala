...
import org.argus.amandroid.plugin.{ApiMisuseModules, InterfaceIntegrationApproach, TaintAnalysisApproach, TaintAnalysisModules}
...
object Main extends App {
  ...
  private def usage(mode: Mode.Value): Unit ={
    ...
    mode match {
      case Mode.ARGUS_SAF =>
        println(s"""Argus-SAF - a static analysis framework for Android apks
  ...
  private def cmdTaintAnalysis(cli: CommandLine): Unit = {
    ...
    TaintAnalysis(module, debug, sourcePath, outputPath, forceDelete, guessPackage, approach, InterfaceIntegrationApproach.MANUAL)
  }
